...
import { friendships, relationships, users } from '$lib/server/database/schema';
...
export async function getUserProfilePhoto(userId: number) {
	...
	const res = await fetch(
		`https://api.telegram.org/file/bot${TELEGRAM_BOT_TOKEN}/${file.filePath}`,
	);
...
bot.on('message', async (message) => {
	...
	if (message.text === '/profile') {
		const userData = await database.query.users.findFirst({
			where: eq(users.telegramId, message.sender!.id),
		});

		if (!userData) return;

		message.chat.sendMessage(`**${userData.name}**\n\n${userData.coins} 🪙\n${userData.rubies}♦️`, {
