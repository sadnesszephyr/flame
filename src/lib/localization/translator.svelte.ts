	...
	for (const stringKey of stringPath) {
		...
		if (typeof resultString === 'string' || !resultString[stringKey]) {
			if (languageData.fallbackLanguageId) {
				console.warn(
					`Couldn't find a translation for key '${key}' in '${languageId}' language, falling back to ${languageData.fallbackLanguageId}`,
				);
...
export const t = getT(localSettings.language);
